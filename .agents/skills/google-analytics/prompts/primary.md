...
type: execution
version: 2.0.0
description: "Execute the Google Analytics workflow with triad orchestration."
triad:
...
3. **Lead** (`google-analytics-lead`): Execute SKILL.md Steps 1-4 for `{{task}}`
   - Discover → Analyze → Execute → Validate
   - Apply evidence tags on all claims
4. **Support** (`google-analytics-support`): Review for cross-cutting concerns
   - Edge cases, security, accessibility, performance
5. **Guardian** (`google-analytics-guardian`): Validate
