...
// ---------------------------------------------------------------------------
export const FORBIDDEN_TERM_MAP = {
...
const FORBIDDEN_PATTERNS_MAP = new Map(Object.keys(FORBIDDEN_TERM_MAP).map((term) => [
    term,
...
// bold/italic tags.
// ---------------------------------------------------------------------------
export function stripFormattingArtifacts(text) {
    if (!text)
        return text;
...
// ---------------------------------------------------------------------------
export function replaceForbiddenTerms(text) {
    if (!text)
        return text;
    let result = text;
...
// ---------------------------------------------------------------------------
export function enforceMintoStructure(text) {
    if (!text || !text.trim())
        return "";
...
// ---------------------------------------------------------------------------
export function scoreDeliverable(text, tier) {
    if (!text || !text.trim()) {
    ...
    const hasForbiddenTerms = Object.keys(FORBIDDEN_TERM_MAP).some((term) => new RegExp(`\\b${term}\\b`, "i").test(text));
    const hasFormattingArtifacts = /\*\*/.test(text) || /\*[^*]+\*/.test(text) || /[\u{1F600}-\u{1F64F}]/u.test(text);
    // Base score influenced by text quality signals
    const baseScore = (() => {
        let score = 7; // neutral baseline
        if (sentences.length >= 3)
            score += 0.5; // has enough supporting content
        if (wordCount >= 30)
            score += 0.5; // sufficient density
        if (!hasForbiddenTerms)
            score += 0.5; // brand compliant
        if (!hasFormattingArtifacts)
            score += 0.5; // format compliant
    ...
    const dimensionScores = EXCELLENCE_DIMENSIONS.map((name, idx) => {
    ...
    const overall = Math.round((dimensionScores.reduce((sum, d) => sum + d.score, 0) / dimensionScores.length) * 10) / 10;
...
// ---------------------------------------------------------------------------
export async function enforceExcellenceLoop(text, tier, scoreFn) {
 ...
 */
function enforceHardEntrust(text) {
    let cleaned = text;
 ...
 */
export function formatForTelegram(markdownText) {
 ...
 */
export function renderDeliverable(templateName, sections) {
 ...
 */
export function chunkDeliverable(text) {
    if (text.length <= DELIVERABLE_CHUNK_LIMIT)
        return [text];
 ...
 */
export function splitMessageHtml(htmlText) {
    if (htmlText.length <= TELEGRAM_MAX_LENGTH)
        return [htmlText];
    const chunks = [];
