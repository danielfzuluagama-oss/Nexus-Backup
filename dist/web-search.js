...
export function normalizeWebSearchText(value) {
    return value
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
...
export function parseWebSearchProvider(value) {
...
function readDistinctNonEmpty(values) {
    const seen = new Set();
    return values
...
function collectGeminiApiKeys(env) {
...
function resolveGeminiWebSearchModel(env) {
    const candidates = [
...
export function resolveWebSearchSettings(env = process.env) {
...
export function resolveActiveWebSearchProvider(settings) {
    if (!settings.enabled) {
        return null;
    }
...
export function getWebSearchIssues(settings) {
    const issues = [];
    if (!settings.enabled) {
...
export function shouldPreferInternetResearch(text) {
...
export function isInternetSearchQueryAllowed(query) {
    ...
    return (includesAnyKeyword(normalized, FRESHNESS_MARKERS)
        || includesAnyKeyword(normalized, SOURCE_VERIFICATION_MARKERS));
}
export function buildInternetResearchResponseContract(text) {
