export const CONTROLLED_DELIVERABLE_TEMPLATES = {
    execution_plan: {
        name: "Execution Plan",
        fields: [
            {
...
export function summarizeControlledTemplate(templateName) {
    const template = getTemplate(templateName);
    return template.fields
        .map((field) => `${field.heading}<=${field.maxLength}c`)
        .join(" | ");
}
export function renderControlledScaffold(templateName, seed = {}) {
    const template = getTemplate(templateName);
    const lines = [`Plantilla controlada: ${template.name}`];
    for (const field of template.fields) {
        lines.push("");
        lines.push(field.heading);
        lines.push(`Limite: ${field.maxLength} caracteres${field.required ? " | obligatorio" : " | opcional"}`);
