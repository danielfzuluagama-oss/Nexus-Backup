...
import { sanitizeInput, SECURITY_INPUT_BLOCKED_MESSAGE, } from "./security.js";
...
import { classifyTelegramIntent, inferAudienceRole, normalizeForMatching, } from "./telegram-intents.js";
import { buildTelegramConversationKey, getTelegramMessageThreadId, } from "./telegram-conversation.js";
import { renderControlledScaffold, } from "./controlled-deliverables.js";
...
import { buildProposalArtifact, extractClientNameFromRequest, formatProposalDate, parseProposalStageSections, slugifyClientName, isGenericCommercialProcessName, } from "./proposals/proposal-artifact.js";
...
import { assessProposalIntake, buildProposalClarificationReply, } from "./proposals/proposal-intake.js";
...
import { isTelegramConversationUpdateStale, recordTelegramConversationUpdate, } from "./telegram-update-guard.js";
import { buildTelegramExecutionId, readTelegramTaskContext, } from "./telegram-task-context.js";
/**
...
function detectProcessMention(normalizedMessage, modules) {
    let bestModule = null;
    let bestScore = 0;
...
function hasExplicitProcessMention(normalizedMessage, module) {
    const candidates = [
        module.processId,
        module.processName,
        ...module.variants,
...
function buildOperationalRisks(module) {
    const risks = [];
...
function formatOnboardingFastPathReply(module, pack) {
    const riskText = buildOperationalRisks(module).join(" | ");
    const assetText = pack.essentialAssets.length > 0
    ...
    const sopText = pack.essentialSops.length > 0
    ...
    const ownerText = module.owners.length > 0 ? module.owners.join(" | ") : "Owners no definidos en la fuente operativa.";
    return [
...
function formatModuleFastPathReply(module) {
    return [
        `Respuesta directa del KB operativo para ${module.processName}.`,
...
function formatStageBlock(title, body) {
...
function buildPublishedProposalReply(clientName, publication) {
...
function buildProposalReadyReply(clientName, serviceName) {
    return [
...
function buildProposalFallbackReply(clientName, hasAttachment = false) {
    return [
...
function compactLogValue(value, maxLength = 180) {
    if (!value) {
        return null;
    }
...
function summarizeProposalValidationIssues(issues) {
...
function shouldRequireProposalIntake(intent) {
    return intent.templateName === "proposal" && intent.kind === "agent";
}
function includesAnyNormalizedKeyword(haystack, keywords) {
    return keywords.some((keyword) => haystack.includes(normalizeForMatching(keyword)));
}
function threadMemorySignalsProposal(threadMemoryContext) {
    const normalizedThreadContext = normalizeForMatching(threadMemoryContext ?? "");
    return (normalizedThreadContext.includes("tipo proposal")
        || normalizedThreadContext.includes("estado comercial"));
}
function hasProposalConversationSignals(recentMessages, threadMemoryContext) {
...
function shouldLoadProposalConversationHistory(currentText, currentIntent, threadMemoryContext) {
...
function shouldPreserveProposalConversationContext(currentText, currentIntent, recentMessages, threadMemoryContext) {
    const normalizedCurrentMessage = currentIntent.normalizedMessage || normalizeForMatching(currentText);
    if (!normalizedCurrentMessage) {
        return false;
    }
...
function resolveProposalClientSlug(text) {
    if (!text) {
        return null;
    }
...
function buildProposalConversationText(currentText, recentMessages, threadMemoryContext) {
...
function buildProposalThreadState(assessment, status) {
...
async function buildProposalKnowledgeContext(text, log, senderName, options = {}) {
    ...
    const detectedModule = detectProcessMention(normalizedMessage, modules)
    ...
    const explicitModuleMention = detectedModule
        ...
        : false;
    const useExecutionPack = Boolean(detectedModule)
        && !genericCommercialModule
        && (!proposalMode || explicitModuleMention);
    const internalProcessCatalog = buildProposalProcessCatalog(text, modules, evidence, useExecutionPack && detectedModule ? detectedModule.processId : null);
    let executionPack = null;
    if (useExecutionPack && detectedModule) {
        executionPack = await kb.createExecutionPack(detectedModule.processId, "propuesta comercial", text);
    }
    let metodologiaKnowledge;
    let metodologiaReady;
    try {
...
function buildExecutionScaffoldSeed(module, pack, templateName) {
...
function formatExecutionFastPathReply(module, pack, templateName) {
    const scaffold = renderControlledScaffold(templateName, buildExecutionScaffoldSeed(module, pack, templateName));
    return [
...
async function tryOperationalRoute(text, intent, log) {
    ...
    try {
        ...
        if (intent.kind === "operational_execution") {
            ...
            const pack = await kb.createExecutionPack(module.processId, intent.deliverable, text);
            if (pack) {
                log.info("Operational fast path resolved", {
                    processId: module.processId,
                    mode: "execution",
 ...
 */
export function createBot(runtime) {
    ...
    bot.on("message", async (ctx) => {
        ...
        const executionId = taskContext?.executionId
            ?? buildTelegramExecutionId(runtime.instanceName, updateId, userId);
        const getEndToEndElapsedMs = () => typeof taskContext?.ingressReceivedAt === "number"
        ...
        let proposalKnowledgeContext = null;
        let proposalIntakeAssessment = null;
        let threadId = null;
        let threadMemoryContext = "";
        let semanticMemoryContext = "";
        const describeThreadMemory = typeof runtime.memory.describeThreadMemory === "function"
            ? runtime.memory.describeThreadMemory.bind(runtime.memory)
            : async () => "";
        const describeSemanticMemory = typeof runtime.memory.describeSemanticMemory === "function"
            ? runtime.memory.describeSemanticMemory.bind(runtime.memory)
            : async () => "";
        const updateThreadMemory = typeof runtime.memory.updateThreadMemory === "function"
        ...
        const isStaleConversationUpdate = async () => {
            try {
                return await isTelegramConversationUpdateStale(runtime.instanceName, conversationKey, updateId);
            }
            catch (error) {
        ...
        const executeIfFresh = async (label, action) => {
        ...
        const replyFresh = (content, options) => executeIfFresh("reply", () => options ? ctx.reply(content, options) : ctx.reply(content));
        const replyWithDocumentFresh = (document, options) => executeIfFresh("reply_document", () => options ? ctx.replyWithDocument(document, options) : ctx.replyWithDocument(document));
        const sendMessageFresh = (targetChatId, content, useHtml = true) => executeIfFresh("send_message", () => useHtml
            ? ctx.api.sendMessage(targetChatId, content, { parse_mode: "HTML" })
            : ctx.api.sendMessage(targetChatId, content));
        const sendDocumentFresh = (targetChatId, document, options) => executeIfFresh("send_document", () => options
            ? ctx.api.sendDocument(targetChatId, document, options)
            : ctx.api.sendDocument(targetChatId, document));
        const sendDirectResponseWithFallback = async (content) => {
...
Confirma recepcion. Ofrece delegar a Document Intelligence para analisis profundo o extraccion de datos.
                ...
                text = "[El usuario ha enviado un GIF animado, asúmelo como un gesto lúdico]";
            else
                text = "[Multimedia genérica no identificada]";
        }
        ...
        const agentTimeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Agent timeout")), timeoutMs));
        ...
        if (currentIntent.kind !== "agent") {
            ...
            if (directResponse) {
                ...
                try {
                    threadId = await runtime.memory.getOrCreateActiveThread(userId, runtime.instanceName, conversationContext);
        ...
        try {
            ...
            const shouldLoadHistory = shouldLoadProposalConversationHistory(text, currentIntent, threadMemoryContext);
            if (shouldLoadHistory) {
                const recentMessages = await runtime.memory.getRecentMessages(userId, 6, threadId, runtime.instanceName);
                if (shouldPreserveProposalConversationContext(text, currentIntent, recentMessages, threadMemoryContext)) {
        ...
        const sendProposalText = async (content, useHtml = true) => {
        ...
        if (proposalMode) {
            semanticMemoryContext = await describeSemanticMemory(userId, proposalConversationText, threadId ?? undefined, runtime.instanceName).catch((error) => {
        ...
        if (shouldRequireProposalIntake(intent)) {
            proposalKnowledgeContext = await buildProposalKnowledgeContext(proposalConversationText, log, ctx.from?.first_name, { proposalMode }).catch((error) => {
        ...
        let proposalDraftJson = null;
        let proposalArtifactClientName = null;
        let resolvedProposalKnowledgeContext = proposalKnowledgeContext ?? null;
        const ensureProposalKnowledgeContext = async (stage) => {
            if (!resolvedProposalKnowledgeContext) {
                const contextStartedAt = Date.now();
                const baseProposalKnowledgeContext = await buildProposalKnowledgeContext(proposalConversationText, log, ctx.from?.first_name, { proposalMode })
                    .catch((error) => {
        ...
        if (proposalMode) {
            try {
                ...
                const candidateProposalArtifact = buildProposalArtifact(proposalConversationText, proposalDraftJson ?? result, new Date(), resolvedProposalKnowledgeContext);
            ...
            if (!proposalArtifact && !proposalArtifactBlocked) {
                ...
                const clientName = proposalArtifactClientName
        ...
        const sendWithFallback = async (replyChunks, useHtml = true) => {
        ...
        if (publishedProposal && proposalArtifact) {
            await sendProposalText(buildPublishedProposalReply(proposalArtifact.clientName, publishedProposal), true).then(() => {
        ...
        if (proposalArtifact) {
            ...
            const sendProposalDocument = deliverProposalPrivately
                ? () => sendDocumentFresh(proposalDeliveryTargetId, proposalDocument, { caption: proposalCaption })
                : () => replyWithDocumentFresh(proposalDocument, { caption: proposalCaption });
