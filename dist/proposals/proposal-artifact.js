import { parseProposalDraft } from "./proposal-draft.js";
import { renderProposalTemplate, } from "./proposal-template.js";
...
function stripUrlArtifacts(value) {
    return compactWhitespace(value)
...
function stripProposalNoise(value) {
    let cleaned = stripInternalOperationalArtifacts(stripUrlArtifacts(stripMarkdownFormatting(value)));
    for (const pattern of PROPOSAL_NOISE_PATTERNS) {
        cleaned = cleaned.replace(pattern, " ");
    }
    return compactWhitespace(stripInternalOperationalArtifacts(cleaned));
}
function stripInternalOperationalArtifacts(value) {
    let cleaned = value;
...
function looksLikeInternalOperationalEntry(value) {
...
export function formatProposalDisplayName(value) {
    const compact = compactWhitespace(value);
    if (!compact) {
        return compact;
    }
    if (compact === SERVICE_PLACEHOLDER_NAME
...
export function isGenericCommercialProcessName(value) {
    const normalized = normalizeForMatching(value);
    return /(^|[\s-])(comercial|presales|preventa|ventas|proposal|oferta|scouting|lead)([\s-]|$)/.test(normalized);
}
function isGenericStructuredDraftLabel(value, kind) {
    const normalized = normalizeForMatching(value);
    if (!normalized) {
        return true;
    }
...
function isLowSignalStructuredDraftText(value, minLength = 32) {
    const normalized = normalizeForMatching(value);
...
function inferServiceNameFromRequest(text) {
...
function sanitizeProposalClientName(value) {
    const compact = compactWhitespace(value);
    if (!compact) {
...
function stripNumericOrBulletPrefix(value) {
    return value
        .replace(/^(?:[-*•]\s+|\d+[.)]\s+)/, "")
        .trim();
}
function takeExcerpt(value, maxLength = 280) {
...
function sanitizeStageContent(value) {
    const blockedPatterns = [
        /Proceso\s+detectado\s*:/i,
        /Entregable\s+solicitado\s*:/i,
        /Resumen\s+del\s+proceso\s*:/i,
...
function extractQuotedClientName(text) {
...
function findSectionByKeywords(sections, keywords) {
    const normalizedKeywords = keywords.map((keyword) => keyword
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase());
...
function mergeKnowledgeChunks(...sources) {
...
function buildReferences(chunks) {
    return chunks.slice(0, 4).map((chunk) => {
        ...
        return {
            title: sanitizeDraftEntry(chunk.title || chunk.sectionTitle || "", 120)
                || "Referencia operativa",
            summary: takeExcerpt(sanitizeSectionContent(stripInternalOperationalArtifacts(chunk.summary || chunk.content)), 180)
                || "Referencia operativa validada para respaldar la propuesta.",
            meta: metaParts.join(" · "),
        };
    });
}
function buildProposalSynopsis(parts, maxLength = 320) {
    const compact = parts
        .map((part) => part?.trim() || "")
        .filter(Boolean)
        .join(" · ");
...
function describeMethodologySource(source) {
...
export function extractClientNameFromRequest(text) {
...
export function extractRequestedServiceName(text) {
...
export function formatProposalDate(date = new Date(), timeZone = "America/Bogota") {
...
export function buildProposalFolderName(clientName, requestDate) {
    return `${slugifyClientName(clientName)}-${requestDate}`;
}
export function parseProposalStageSections(text) {
...
function buildProposalTemplateModel(requestText, clientName, requestDate, sections, context = {}, draft = null) {
    const structuredDraft = draft ?? null;
    const summarySection = findSectionByKeywords(sections, ["repaso", "entendido"])
        ?? sections[0];
    const planSection = findSectionByKeywords(sections, ["plan", "accion", "acción"])
        ?? sections[1]
        ?? summarySection;
    const scaffoldSection = findSectionByKeywords(sections, ["scaffold", "inicial", "propuesta", "solucion", "solución"])
        ?? sections[2]
        ?? planSection;
    const nextStepSection = findSectionByKeywords(sections, ["siguiente", "paso", "cierre"])
        ?? sections.at(-1)
        ?? scaffoldSection;
    ...
    const executionPack = context.executionPack && !isGenericCommercialProcessName(context.executionPack.processName)
        ? context.executionPack
        : null;
    const primaryInternalProcess = proposalProcessCatalog.find((item) => item.isPrimary)
        ?? proposalProcessCatalog.find((item) => !isGenericCommercialProcessName(item.processName))
        ?? null;
    const mergedEvidence = mergeKnowledgeChunks(executionPack?.evidence, context.evidence);
    const references = buildReferences(mergedEvidence);
    const serviceName = compactWhitespace(formatProposalDisplayName(stripUrlArtifacts(structuredDraft?.serviceName?.trim()
    ...
    const processName = compactWhitespace(stripUrlArtifacts(structuredDraft?.processName?.trim()
    ...
    const catalogAssets = sanitizePublicEntries(proposalProcessCatalog.flatMap((item) => item.assets), 180).slice(0, 8);
    const catalogSops = sanitizePublicEntries(proposalProcessCatalog.flatMap((item) => item.sops), 180).slice(0, 8);
    const catalogGates = sanitizePublicEntries(proposalProcessCatalog.flatMap((item) => item.gates), 180).slice(0, 6);
    const catalogPhases = sanitizePublicEntries(primaryInternalProcess?.phases.map((phase, index) => `${index + 1}. ${phase}`) ?? [], 180);
    const steps = dedupe([
    ...
    const challengeFallback = takeExcerpt(sanitizeSectionContent([
        intakeObjective,
        ...
        .filter(Boolean)
        .join(" ")), 420);
    const approachFallback = takeExcerpt(sanitizeSectionContent([
        ...
        .filter(Boolean)
        .join(" ")), 420);
    const nextStepFallback = sanitizeSectionContent(intakeNextStep
    ...
    const model = {
        ...
        processName,
        requestDate,
        summary: (structuredDraft?.summary ? sanitizeDraftEntry(structuredDraft.summary, 320) : "")
            || synopsis
            || takeExcerpt(sanitizeSectionContent(primaryInternalProcess?.summary
        ...
        challenge: structuredDraft?.challenge
            ...
            : takeExcerpt(sanitizeSectionContent(intakeObjective
                || [
                    intakeScope ? `Alcance: ${intakeScope}` : "",
                    intakeTimeline ? `Cronograma: ${intakeTimeline}` : "",
                    challengeFallback,
        ...
        approach: structuredDraft?.approach
            ...
            : takeExcerpt(sanitizeSectionContent([
                intakeScope ? `Alcance: ${intakeScope}` : "",
                intakeTimeline ? `Cronograma: ${intakeTimeline}` : "",
        ...
        metodologiaFounders: (() => {
            ...
            const preferred = context.requestedByFounder
                ? founders.filter((f) => normalizeForFounderMatch(f.name).includes(requestedFounder))
                : founders;
...
function buildDraftSections(draft) {
...
function normalizeForFounderMatch(value) {
    return compactWhitespace(value)
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase();
}
function stripStructuredDraftNoise(value) {
    return compactWhitespace(stripProposalNoise(value)
        .replace(/\bETAPA\s+\d+\s*\|\s*[^.\n]+/gi, " ")
        .replace(/\bETAPA\s+\d+\b/gi, " ")
        .replace(/\s{2,}/g, " "));
}
function sanitizeDraftEntry(value, maxLength = 160) {
...
function sanitizeDraftEntries(values, maxLength = 160) {
    return dedupe(values
        .map((value) => sanitizeDraftEntry(value, maxLength))
        .filter(Boolean));
}
function buildFallbackSectionsFromDraft(draft) {
    return [
        {
...
function normalizeStructuredDraft(draft, context) {
    const intake = context.intake ?? {};
    const primaryProcess = context.internalProcessCatalog?.find((item) => item.isPrimary)
        ?? context.internalProcessCatalog?.[0];
    const fallbackServiceName = compactWhitespace(intake.serviceName
        ?? context.serviceName
        ?? context.executionPack?.deliverable
        ?? primaryProcess?.processName
        ?? draft.serviceName) || SERVICE_PLACEHOLDER_NAME;
    const fallbackProcessName = compactWhitespace(context.executionPack?.processName
        ?? primaryProcess?.processName
        ?? draft.processName) || PROCESS_PLACEHOLDER_NAME;
    const fallbackClientName = sanitizeProposalClientName(intake.clientName
        ?? draft.clientName
    ...
    const fallbackNextStep = takeExcerpt(intake.nextStep
        ?? `Validar alcance, responsables y fecha de arranque para ${fallbackServiceName}.`, 240);
    ...
    const normalizedDraft = {
        ...
        assets: assets.length > 0
            ? assets
            : sanitizePublicEntries([...(context.executionPack?.assets ?? []), ...(primaryProcess?.assets ?? [])], 180).slice(0, 8),
        sops: sops.length > 0
            ? sops
            : sanitizePublicEntries([...(context.executionPack?.sops ?? []), ...(primaryProcess?.sops ?? [])], 180).slice(0, 8),
        gates: gates.length > 0
            ? gates
            : sanitizePublicEntries([...(context.executionPack?.gates ?? []), ...(primaryProcess?.gates ?? [])], 180).slice(0, 6),
...
function sanitizePublicEntries(values, maxLength = 160) {
    return dedupe(values
        .map((value) => sanitizeDraftEntry(value, maxLength))
        .map(stripNumericOrBulletPrefix)
...
export function buildProposalArtifact(requestText, proposalText, generatedAt = new Date(), context = {}) {
    ...
    const clientName = sanitizeProposalClientName(context.intake?.clientName
