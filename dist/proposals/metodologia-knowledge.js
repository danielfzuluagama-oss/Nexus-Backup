...
let cachedKnowledgePromise = null;
...
function formatOfferSummary(offers) {
    if (!isRecord(offers))
        return "";
...
function extractPersonCandidates(documents) {
    const candidates = new Map();
    const pushCandidate = (record) => {
        if (!isRecord(record))
            return;
...
function collectFounderBio(visibleLines, name, title, allFounderNames) {
    ...
    for (let index = nameIndex + 1; index < visibleLines.length; index++) {
        ...
        if (normalized === "ceo")
            continue;
        if (normalized === titleMarker)
            continue;
        if (otherFounders.has(normalized) && normalized !== normalizeForMatch(name))
            break;
        if ([...stopMarkers].some((marker) => normalized.includes(marker)))
            break;
        collected.push(line);
        if (collected.join(" ").length >= 320)
            break;
    }
    return compactWhitespace(collected.join(" "));
}
function extractFounders(html, documents) {
...
function extractServices(html, documents) {
    const services = [];
    for (const document of documents) {
        if (!isRecord(document))
            continue;
        ...
        if (!isItemList || !Array.isArray(document.itemListElement))
            continue;
...
function extractResources(html, documents) {
    const resources = [];
    for (const document of documents) {
        if (!isRecord(document))
            continue;
        ...
        if (!isCollectionPage || !Array.isArray(document.hasPart))
            continue;
...
export async function loadMetodologiaKnowledge(fetcher = defaultFetch) {
    ...
    cachedKnowledgePromise = (async () => {
        ...
        const tryFetch = async (url, attempts = 2) => {
