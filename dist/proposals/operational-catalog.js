...
function pickTop(values, limit) {
    return dedupe(values).slice(0, limit);
}
function buildSearchSpace(module) {
...
function scoreModule(module, normalizedMessage, queryTokens, evidenceProcessIds, primaryProcessId) {
...
function buildMatchReason(ranked) {
...
export function buildProposalProcessCatalog(message, modules, evidence, primaryProcessId, limit = 4) {
    const normalizedMessage = normalizeText(message);
    const queryTokens = tokenize(normalizedMessage);
    const evidenceProcessIds = new Set(evidence
        .map((chunk) => chunk.processId)
        .filter((processId) => Boolean(processId)));
    const ranked = modules
        .map((module) => scoreModule(module, normalizedMessage, queryTokens, evidenceProcessIds, primaryProcessId))
