...
function renderListRows(items, emptyLabel) {
    const values = items.length > 0 ? items : [emptyLabel];
    return values
        .map((item) => `
...
function renderReferenceCards(items, emptyTitle, emptySummary) {
...
function renderStageCards(sections) {
...
function renderIntroLetter(model) {
...
function renderHeroContent(model, primaryMailto) {
    return `
...
function renderHookContent(model) {
...
function renderVisionContent(model) {
    return `
    <div class="grid lg:grid-cols-2 gap-16 items-center">
...
function groupProgramCards(model) {
    const icons = [
        "target",
        "route",
        "package",
...
function renderProgramaContent(model) {
...
function renderModalidadesContent(model) {
    const cards = [
...
function renderObjecionesContent(model) {
...
function renderCredencialesContent(model) {
    return `
    <div class="text-center mb-16 reveal">
...
function renderEquipoContent(model) {
...
function renderPristinoContent(model) {
    const supportChips = [
        model.processName,
...
function renderMetodologiasContent(model) {
    return `
    <div class="grid lg:grid-cols-2 gap-16 items-center">
      <div class="reveal">
...
function renderRoiContent(model) {
...
function renderConfiguradorContent(model, primaryMailto) {
    return `
    <div class="text-center mb-16 reveal">
...
function renderServiciosContent(model) {
    const extras = [
...
function renderConditionsTable(model) {
    const rows = [
...
function renderCondicionesContent(model) {
    return `
    <div class="text-center mb-16 reveal">
...
function renderStackContent(model) {
    const rows = [
...
function renderWorkshopContent(model) {
    return `
    <div class="text-center mb-16 reveal">
...
function renderWorkshopCtaContent(model, primaryMailto) {
    return `
...
function renderGlossaryContent(model) {
    const entries = [
        {
            term: "Cliente",
...
function renderFinalSection(model, primaryMailto) {
    return `
...
function renderFooterContent(primaryMailto) {
    return `
...
function renderGenericModal(title, body, linkId, linkHref, linkLabel, modalName) {
    return `
...
function buildMailto(email, subject, bodyLines) {
    return `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyLines.join("\n"))}`;
}
function buildPrimaryMailto(model) {
    return buildMailto(DEFAULT_CONTACT_EMAIL, `Propuesta comercial · ${model.clientName} · ${model.serviceName}`, [
...
function injectPayload(templateHtml, payload) {
    const payloadTagPattern = /(<script id="proposal-template-payload" type="application\/json">)([\s\S]*?)(<\/script>)/;
...
function formatInlineScriptAssignment(name, value) {
    const serialized = JSON.stringify(value, null, 2).replace(/^/gm, "  ");
    return `  let ${name} = ${serialized};\n`;
}
function replaceInlineScriptAssignment(html, name, endMarker, value) {
...
function scrubEmbeddedTemplateDefaults(html, payload) {
    let output = html;
    output = replaceInlineScriptAssignment(output, "runtimeConfig", "\n\n  let infoData = ", payload.runtimeConfig);
    output = replaceInlineScriptAssignment(output, "infoData", "\n\n  /* ==================================================================\n     DATA: 9 MODULES", payload.infoData);
    output = replaceInlineScriptAssignment(output, "modulesData", "\n\n  /* ==================================================================\n     ASSISTANT NAMES", payload.modulesData);
    output = replaceInlineScriptAssignment(output, "assistantNames", "\n\n  /* ==================================================================\n     i18n DICTIONARIES", payload.assistantNames);
    output = replaceInlineScriptAssignment(output, "i18n", "\n\n  let i18nHtml = ", payload.i18n);
    output = replaceInlineScriptAssignment(output, "i18nHtml", "\n\n  /* ==================================================================\n     APPLICATION STATE", payload.i18nHtml);
    return output;
}
function applyServerSideSelectorContent($, map, mode) {
...
function applyServerSideAttributeBindings($, map) {
...
function applyPayloadServerSide(html, payload) {
    const $ = loadHtml(html);
    ...
    const output = scrubEmbeddedTemplateDefaults($.html(), payload).replace(/^<!doctype html>/i, "<!doctype html>");
    return output.startsWith("<!doctype html>") ? output : `<!doctype html>\n${output}`;
}
export function renderProposalTemplate(model) {
