...
Feature: Ecosystem Agent Loading
  ...
  Scenario: Valid agent definition loaded and registered
    Given a valid agent definition file exists
    When the ecosystem loader runs at startup
    Then it parses the definition and validates all required fields
    And registers the agent for routing

  @TS-026 @FR-007 @P1 @acceptance
  Scenario: Invalid agent definition skipped with warning
    Given an agent definition is missing required fields
    When the loader validates it
    Then it logs a warning and skips the invalid agent
    And the system continues without crashing

  @TS-027 @FR-008 @P1 @acceptance
  Scenario: Shared defaults merged into agent definition
    Given shared defaults exist
    When an agent definition omits a field covered by defaults
    Then the loader merges the default value

  @TS-028 @FR-010 @P1 @acceptance
  Scenario: At least 6 concurrent agents supported
    Given 6 or more valid agent definition files exist
    When the ecosystem loader processes all of them
    Then all agents are registered with independent skill catalogs
  ...
  Scenario: AgentDefinition Zod schema rejects missing required fields
    ...
    Then a validation error is returned listing the missing field
