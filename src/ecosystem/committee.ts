...
export async function executeCommittee(
  ...
  agents: AgentDefinition[],
  task: string,
  timeoutMs: number = COMMITTEE_TIMEOUT_MS
): Promise<CommitteeResult> {
  if (agents.length === 0) {
    const fallback = await runner(task, "You are a helpful assistant.", []);
    return {
  ...
  const resolvedInOrder: Array<{ agentId: string; response: string }> = [];
  ...
  const raceResult = await Promise.race([
    Promise.allSettled(agentPromises).then((r) => ({ kind: "done" as const, results: r })),
  ...
  let tiebreaker: string | null = null;
  let tiebreakerCriterion: string | undefined;

  if (deliberations.length >= 3) {
    const tiebreakerResult = await runFactualTiebreaker(
      runner,
      deliberations,
      synthesis,
      task
...
function computeConsensusStatus(
  ...
  if (uniquePositions === 1) return "strong";
  if (maxVotes / total >= 0.8) return "strong";
  if (maxVotes / total >= 0.6) return "majority";
  if (uniquePositions === 2 && total >= 4) return "split";
  return "disagreement";
}
 ...
 */
async function runFactualTiebreaker(
  runner: SubAgentRunner,
  deliberations: Array<{ agentId: string; response: string }>,
  synthesis: string,
  originalTask: string
