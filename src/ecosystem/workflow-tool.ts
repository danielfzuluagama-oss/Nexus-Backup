...
import type { ToolDefinition, ToolExecutor } from "../tools/registry.js";
...
interface WorkflowLookupRequest {
  agentId?: string;
  skillId?: string;
  workflowId?: string;
  query?: string;
...
function normalizeContext(value: unknown): Record<string, string> {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .map(([key, entry]) => [key.trim(), toStringValue(entry)] as const)
...
function buildWorkflowSearchText(binding: WorkflowBinding): string {
  return [
    binding.agent.id,
    binding.agent.name,
    binding.agent.role,
...
function collectWorkflowBindings(ecosystem: EcosystemState): WorkflowBinding[] {
...
function scoreWorkflowBinding(binding: WorkflowBinding, query: string): number {
  ...
  if (
    normalizedQuery === binding.workflow.id.toLowerCase()
...
function resolveWorkflowBinding(
  ...
  request: WorkflowLookupRequest,
): { binding: WorkflowBinding | null; suggestions: WorkflowBinding[] } {
...
function formatSuggestions(suggestions: WorkflowBinding[]): string {
  ...
  return suggestions
    .map((binding) =>
      `- ${binding.agent.id}/${binding.skill.id}/${binding.workflow.id}: ${binding.workflow.title}`
    )
    .join("\n");
}

function buildWorkflowPrompt(binding: WorkflowBinding): string {
...
function deriveAllowedTools(binding: WorkflowBinding): string[] {
...
function extractArgumentName(actionInstruction: string, fnName: string): string | null {
  const match = actionInstruction.match(new RegExp(`${fnName}\\(([^)]+)\\)`, "i"));
  const raw = match?.[1]?.trim();
  return raw ? raw.replace(/[{}'"]/g, "").trim() : null;
}

function inferTimeOfDay(now: Date): string {
...
function inferTimezone(actionInstruction: string, context: Record<string, string>): string {
...
async function executeKnownMechanicalStep(
  step: StepDefinition,
  context: Record<string, string>,
): Promise<{ output: string; contextPatch: Record<string, string>; handoff: string | null } | null> {
    ...
    if (actionLower.includes("for each timezone in the list")) {
      ...
      if (timezones.length > 0) {
        const entries = await Promise.all(
          timezones.map(async (item) => `${item}: ${await executeTool("get_current_time", { timezone: item })}`)
        );
...
export function getWorkflowExecutionDefinition(
  ecosystem: EcosystemState,
): ToolDefinition {
  ...
  return {
    ...
    function: {
      ...
      description:
        "Execute a hydrated skill workflow from the Pristino ecosystem. " +
...
export function createWorkflowExecutor(
  ...
  return async (args: Record<string, unknown>): Promise<string> => {
    ...
    const workflowRunner: SubAgentRunner = (stepTask, stepPrompt, stepAllowedTools) => {
    ...
    const result = await executeWorkflowDetailed(
      workflowRunner,
      binding.workflow,
      seededContext,
      {
