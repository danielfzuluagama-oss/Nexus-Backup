...
import { registerTool, ToolDefinition } from './registry.js';

interface OpenClawSkill {
  namespace: string;
  name: string;
  type: "executable" | "mcp_server" | "local_js";
  ...
  status: "active" | "pending_installation";
}

interface OpenClawRegistry {
  version: string;
  description: string;
  skills: OpenClawSkill[];
}

export async function initializeOpenClawSymlinks(registryPath: string): Promise<void> {
  ...
  if (fs.existsSync(registryPath)) {
    try {
      ...
      const registry: OpenClawRegistry = JSON.parse(rawData);
      ...
      for (const skill of registry.skills) {
        ...
        const executor = async (args: Record<string, unknown>): Promise<string> => {
