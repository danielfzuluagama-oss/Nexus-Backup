...
export const definition: ToolDefinition = {
  ...
  function: {
    ...
    description:
      "Build an onboarding pack for a MetodologIA operational process so Nexus can onboard a person, team, or client with evidence and process structure.",
    parameters: {
      type: "object",
      properties: {
        process_id: {
...
export async function execute(args: Record<string, unknown>): Promise<string> {
  ...
  const audienceRole =
    typeof args.audience_role === "string" && args.audience_role.trim()
  ...
  try {
    ...
    const pack = await kb.createOnboardingPack(
      processId,
      audienceRole,
      objective,
    );
