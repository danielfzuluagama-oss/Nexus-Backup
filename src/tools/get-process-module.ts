...
export const definition: ToolDefinition = {
  ...
  function: {
    ...
    description:
      "Return the operational module for a MetodologIA process, including phases, gates, assets, SOPs, metrics, and execution capabilities.",
    parameters: {
      type: "object",
      properties: {
        process_id: {
  ...
  try {
    ...
    if (!processId) {
      ...
      return processes
        .map(
          (process, index) =>
            `${index + 1}. ${process.processId} — ${process.processName}\nEstado: ${process.status}\nResumen: ${process.summary}`,
        )
        .join("\n\n");
    }
