...
function compactModuleForStorage(module: ProcessModule): ProcessModule {
  return {
    ...module,
...
export class OperationalKnowledgeStore {
  ...
  async hasSyncedKnowledge(): Promise<boolean> {
  ...
  async getReport(): Promise<MigrationReport> {
    ...
    return doc.data()?.report as MigrationReport;
  }
  ...
  async listProcesses(): Promise<ProcessModule[]> {
    ...
    return snapshot.docs
      .map((doc) => doc.data().module as ProcessModule)
  ...
  async resolveProcess(processIdOrName: string): Promise<ProcessModule | null> {
    ...
    if (exact.exists) {
      return exact.data()?.module as ProcessModule;
    }

    const modules = await this.listProcesses();
    const direct =
      modules.find((module) => module.processId === query) ??
  ...
  async search(query: string, options: SearchOptions = {}): Promise<KnowledgeChunk[]> {
    const db = this.requireDb();
    ...
    const collectSnapshot = async (snapshot: QuerySnapshot) => {
  ...
  async syncBundle(
    bundle: OperationalKnowledgeBundle,
    options: { pruneExisting?: boolean } = {},
    ...
    if (pruneExisting) {
      prunedChunks = await this.deleteQueryInBatches("rag_chunks", (firestore) =>
        firestore.collection("rag_chunks").where("userId", "==", KB_SYSTEM_USER_ID),
      );
      prunedModules = await this.deleteQueryInBatches(KB_MODULES_COLLECTION, (firestore) =>
        firestore.collection(KB_MODULES_COLLECTION),
      );
    }
...
let singleton: OperationalKnowledgeStore | null = null;

export function getOperationalKnowledgeStore(): OperationalKnowledgeStore {
  if (!singleton) {
...
export function getOperationalKnowledgeSystemUserId(): string {
  return KB_SYSTEM_USER_ID;
}

export function getOperationalKnowledgeBaseId(): string {
  return KB_ID;
}
