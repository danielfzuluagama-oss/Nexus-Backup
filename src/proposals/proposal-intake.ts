import type { ProposalKnowledgeContext } from "./proposal-artifact.js";
import {
...
export type ProposalIntakeFieldId =
  | "client_name"
  | "service_name"
  | "objective"
  | "geography"
...
export interface ProposalIntakeSnapshot {
...
export interface ProposalIntakeField {
  id: ProposalIntakeFieldId;
  label: string;
  prompt: string;
  required: boolean;
  value: string | null;
...
export interface ProposalIntakeAssessment {
  isReady: boolean;
  completenessScore: number;
  snapshot: ProposalIntakeSnapshot;
  fields: ProposalIntakeField[];
  missingRequiredFields: ProposalIntakeField[];
  missingRecommendedFields: ProposalIntakeField[];
}
...
function stripObjectiveNoise(value: string): string {
...
function takeExcerpt(value: string | null | undefined, maxLength = 140): string | null {
...
function collectHintMatches(text: string, hints: string[]): string[] {
...
function extractObjective(text: string, context: ProposalKnowledgeContext): string | null {
  ...
  const labeledServiceObjective = takeExcerpt(
    stripObjectiveNoise(labeledServiceMatch?.[1] ?? ""),
    160,
  );
  if (
    labeledServiceObjective
  ...
  const focusedClause = cleanedClauses.find((clause) =>
    OBJECTIVE_FOCUS_PATTERNS.some((pattern) => pattern.test(clause)),
  );
...
function extractScope(text: string): string | null {
  ...
  const participantPattern =
    /\b(?:para|con)\s+(\d{1,3}\s*(?:personas?|participantes?|lideres?|líderes?|equipos?))/i;
...
function extractInvestment(text: string): string | null {
...
function extractNextStep(text: string): string | null {
  const explicitPattern =
    /\b(?:siguiente paso|next step|cierre esperado)\s*[:\-]\s*([^\n.]{8,180})/i;
  ...
  const stepPattern =
    /\b(?:reunion|reunion|llamada|presentacion|presentacion|version final|contrato|firma|aprobacion|aprobacion)\b[^.\n]{0,90}/i;
...
export function assessProposalIntake(
  text: string,
  context: ProposalKnowledgeContext = {},
): ProposalIntakeAssessment {
  const snapshot = buildSnapshot(text, context);
  const fields: ProposalIntakeField[] = [
    {
      id: "client_name",
      label: "Cliente",
  ...
  const weightedPresent = fields.reduce(
    (sum, field) => sum + (field.value ? (field.required ? 2 : 1) : 0),
    0,
  );
...
export function buildProposalClarificationReply(
  ...
  for (const field of missingFields) {
    ...
    lines.push(
      `- <b>${escapeHtml(field.label)}:</b> ${escapeHtml(field.prompt)} ${suffix}`,
    );
  }

  lines.push(
    "",
    "Puedes responder en un solo mensaje usando este formato:",
  );
  ...
  lines.push(
    "",
    "Si aun no quieres fijar inversion, responde \"pendiente\" y genero la propuesta sin ese numero final.",
  );
