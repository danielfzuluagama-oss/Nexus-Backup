...
import {
  buildDraftResponseContract,
  parseProposalDraft,
  type ProposalDraft,
} from "./proposal-draft.js";

interface CommercialProposalComposerArgs {
  userId: number;
  requestText: string;
  knowledgeContext: ProposalKnowledgeContext;
  agentName: AgentName;
}
...
function buildCommercialProposalSystemPrompt(): string {
  return [
...
function formatKnowledgeContext(context: ProposalKnowledgeContext): string {
...
function buildDraftTask(requestText: string, knowledgeContext: ProposalKnowledgeContext): string {
  return [
...
function isGenericDraftLabel(value: string, kind: "client" | "service" | "process"): boolean {
...
function preferSpecificText(candidate: string | undefined, fallback: string, minimumLength: number): string {
...
function buildFallbackSourceMap(knowledgeContext: ProposalKnowledgeContext): ProposalDraft["sourceMap"] {
...
function buildFallbackDraft(knowledgeContext: ProposalKnowledgeContext): ProposalDraft {
  const intake = knowledgeContext.intake ?? {};
  const executionPack = knowledgeContext.executionPack;
  const primaryProcess =
    knowledgeContext.internalProcessCatalog?.find((item) => item.isPrimary)
  ...
  const serviceName =
    ...
    || "Propuesta comercial";
  const processName =
    executionPack?.processName?.trim()
    || primaryProcess?.processName?.trim()
    || "Proceso comercial";
  const clientName =
...
function canUseDeterministicDraft(knowledgeContext: ProposalKnowledgeContext): boolean {
  const intake = knowledgeContext.intake ?? {};
  return Boolean(
    knowledgeContext.executionPack
    && (
      intake.serviceName?.trim()
      || knowledgeContext.serviceName?.trim()
...
async function generateDraftOnce(
  ...
  const commercialDeps: AgentDeps = {
    ...deps,
...
function normalizeDraftAgainstContext(
  draft: ProposalDraft,
  knowledgeContext: ProposalKnowledgeContext,
): ProposalDraft {
  ...
  return {
    clientName:
      draft.clientName.trim() && !isGenericDraftLabel(draft.clientName, "client")
        ? draft.clientName.trim()
        : fallback.clientName,
    serviceName:
      draft.serviceName.trim() && !isGenericDraftLabel(draft.serviceName, "service")
        ? draft.serviceName.trim()
        : fallback.serviceName,
    processName:
      draft.processName.trim() && !isGenericDraftLabel(draft.processName, "process")
...
export async function composeCommercialProposalDraft(
  deps: AgentDeps,
  args: CommercialProposalComposerArgs,
  ...
  try {
    ...
    let draftText = await generateDraftOnce(
      deps,
      args.userId,
      args.agentName,
      firstPrompt,
    ...
    if (!parsed) {
      ...
      draftText = await generateDraftOnce(
        deps,
        args.userId,
        args.agentName,
        repairPrompt,
