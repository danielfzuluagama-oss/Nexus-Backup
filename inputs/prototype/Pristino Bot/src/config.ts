...
/**
 * Per-agent credentials with multiple API keys for cascading rotation.
 * Keys ordered by priority; providers iterate them on quota exhaustion (429).
 */
...
export interface Config {
  /** @deprecated Use agentCredentials instead for multi-agent setups */
  telegramBotToken: string;
  allowedUserIds: number[];
  /** @deprecated Use agentCredentials.groqApiKeys[0] */
  groqApiKey: string;
  groqModel: string;
  groqModelTiers: string[];
 ...
 */
function collectOrderedKeys(prefix: string, agentSuffix: string): ApiKey[] {
  const keys: Array<{ priority: number; value: string; owner: string }> = [];
  const multiKeyPattern = new RegExp(`^${prefix}_${agentSuffix}_(\\d+)_?(\\w+)?`, "i");
  const legacyKey = `${prefix}_${agentSuffix}`;

  for (const [envKey, envValue] of Object.entries(process.env)) {
    if (!envValue || envValue === "<PENDING>" || !envValue.trim()) continue;
...
export function loadConfig(): Config {
  ...
  if (allowedUserIds.length === 0) missing.push("TELEGRAM_ALLOWED_USER_IDS");
...
export function getInstanceDbPath(config: Config, agentName: AgentName): string {
  if (process.env.DB_PATH) return config.dbPath;
