...
function extractListOrUndef(body: string, heading: string): string[] | undefined {
...
const REQUIRED_STRING_FIELDS: (keyof AgentDefinition)[] = [
...
const REQUIRED_ARRAY_FIELDS: (keyof AgentDefinition)[] = [
...
function validateAgentDefinition(agent: AgentDefinition): string[] {
  const errors: string[] = [];
  for (const f of REQUIRED_STRING_FIELDS) {
    if (!agent[f]) errors.push(`Empty field: ${f}`);
  }
  for (const f of REQUIRED_ARRAY_FIELDS) {
    const arr = agent[f] as string[];
    if (!arr || arr.length === 0) errors.push(`Empty array: ${f}`);
  }
  return errors;
}
...
/** Shared defaults loaded from agents/_shared/defaults.yaml (if present). */
export interface SharedDefaults {
  securityValidations: string[];
  defaultObservabilityEvents: string[];
  defaultFailureHandling: string[];
}

let cachedDefaults: SharedDefaults | null = null;

/** Load shared defaults from _shared/defaults.yaml. Cached after first load. */
export function loadSharedDefaults(agentsPath: string): SharedDefaults {
  if (cachedDefaults) return cachedDefaults;

  const defaultsPath = join(agentsPath, "_shared", "defaults.yaml");
  const fallback: SharedDefaults = {
    securityValidations: [
  ...
  try {
    ...
    const extractYamlList = (key: string): string[] => {
...
export function loadAllAgents(agentsPath: string): EcosystemState {
  ...
  for (const dir of dirs) {
    if (dir.startsWith("_")) continue; // skip _shared and other meta-dirs
