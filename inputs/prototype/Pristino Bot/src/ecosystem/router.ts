...
export function getRouteRequestDefinition(
  ...
  return {
    ...
    function: {
      name: "route_request",
      description:
        "Route a user request to specialist agent(s). Choose the appropriate mode:\n" +
      ...
      parameters: {
        ...
        properties: {
          mode: {
            type: "string",
            description:
              'Routing mode: "single", "terna", or "committee"',
          },
          agents: {
            type: "array",
            items: { type: "string" },
            description:
              "Agent IDs to involve (1 for single, 3 for terna, all for committee)",
          },
          task: {
            type: "string",
...
export function createRouteExecutor(
  ...
  return async (args: Record<string, unknown>): Promise<string> => {
    ...
    const decision: RoutingDecision = {
      mode: mode as RoutingDecision["mode"],
      agents,
      reason,
      reversible: true,
    };
...
async function executeCommittee(
  ecosystem: EcosystemState,
  runner: SubAgentRunner,
  agentIds: string[],
  task: string
